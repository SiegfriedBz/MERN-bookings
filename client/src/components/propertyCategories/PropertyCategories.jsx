    ...
    return(
        <div className="pl--container">
            {propertyCategoryList &&
                propertyCategoryList.map(pc => {
                    const { id, img, category, count } = pc
                    return (
                        <div key={id} className="pl--item">
                            <img className="pl--image" src={img} alt="property category"/>
                            <h4 className="pl--text">{category}</h4>
                            <p className="pl--text">{count} {category.toLowerCase()}</p>
                        </div>
                    )
                })
            }
